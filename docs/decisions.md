...
## ADR-018 — Paiements : checkout Mobile Money provider-agnostic (PawaPay / FedaPay / PayDunya)

- **Date** : 2026-09-05
- **Contexte** : ADR-008 posait l'abstraction `PaymentProvider` (Mobile Money d'abord) ; les tables `plans`/`payments` existent depuis 00005. Il faut vendre des packs de crédits aux marchés CEEAC/CEDEAO avec les comptes disponibles : PawaPay, FedaPay, PayDunya — un seul actif à la fois, choisi par env.
- **Options** : (A) SDKs/librairies officielles de chaque provider ; (B) intégration REST directe derrière le port `PaymentProvider`, un seul provider actif via `PAYMENT_PROVIDER`.
- **Décision** : **(B)** — port `port.PaymentProvider` : `CreateCheckout` (page hébergée + référence provider), `VerifyStatus` (source de vérité par API) et `HandleWebhook` (parsing provider). Adapters : `infra/payments/pawapay` (POST `/v2/checkouts` hébergé, statuts COMPLETED/FAILED/CANCELLED/EXPIRED, callback JSON), `infra/payments/fedapay` (POST `/v1/transactions` + `/token` → URL checkout, statuts approved/declined/canceled, webhook Event JSON `transaction.approved`…), `infra/payments/paydunya` (checkout-invoice avec 3 headers PAYDUNYA-*, IPN form-urlencoded, confirmation `checkout-invoice/confirm/{token}` avec contrôle `hash` = SHA-512(master key)). **Règle de sécurité commune** : le corps d'un webhook n'est jamais une preuve — chaque notification est reconfirmée par `VerifyStatus` avant octroi ; les crédits sont accordés par le ledger idempotent (référence `payment:<id>`, jamais de double octroi) ; le retour navigateur (`?payment=`) ne fait que déclencher une resynchronisation. Montants : `payments.amount_minor` (XOF = zéro décimale). Migration `00019` : `payments.checkout_url` + seed idempotent de 3 packs (50 cr/5 000 XOF, 120 cr/10 000, 350 cr/25 000). Endpoints : `GET /payments/plans`, `POST /payments/checkout` (202), `GET /payments/{id}`, `POST /payments/{id}/sync`, webhook public `POST /payments/webhook/{provider}` (sans JWT — statut reconfirmé). Sans `PAYMENT_PROVIDER` (ou clé manquante), le module est désactivé et l'application démarre normalement (plans renvoient `enabled=false`).
- **Conséquences** : `PAWAPAY_API_TOKEN/COUNTRY`, `FEDAPAY_SECRET_KEY`, `PAYDUNYA_*` à définir dans l'environnement ; les webhooks des providers sont configurés dans leurs dashboards vers `/api/v1/payments/webhook/{provider}` ; multi-provider simultané = évolution (registry déjà en place) ; pas de remboursement au MVP.
