...
- [x] **Correctif checkout PawaPay (prod)** : champ `reason` du checkout hébergé refusé par PawaPay (`INVALID_PARAMETER` — ASCII alphanumérique + espaces uniquement) → `pawaReason()` (`infra/payments/pawapay/pawapay.go`) translittère les accents, remplace la ponctuation par des espaces et tronque à 100 car. ; tests `TestPawaReason` + assertion `reason` dans le httptest de création de checkout. Déploiement Railway : Dockerfile backend `golang:1.23-alpine` → `1.24-alpine` (go.mod exige Go 1.24 via SDK AWS v2 — build échouait) ; provider de paiement activé en **sandbox** sur la prod Railway (`PAYMENT_PROVIDER=pawapay`, `APP_URL`, `PAWAPAY_API_URL=https://api.sandbox.pawapay.io`, token + webhook) via `backend/.env.railway` (gitignoré) ; passage en live = swap token + `PAWAPAY_API_URL`.
